document.addEventListener('DOMContentLoaded',()=>{
  ...
  const form=$('#talep'), result=$('.brief-result');
  let saveWorks=true;
  ...
  function invalidate(){result.hidden=true;$('#brief-whatsapp').removeAttribute('href');$('#brief-output').value='';$('#copy-status').textContent='';}
  function updateButtons(){
    $$('[data-program-id]').forEach(button=>{
      ...
      if(button.classList.contains('card-add'))button.innerHTML=selected?'✓':'+';
  ...
  function render(){
    ...
    $('#program-dock').hidden=!items.length;document.body.classList.toggle('has-program',items.length>0);
  ...
  function changed(message){save();invalidate();render();$('#program-status').textContent=message;$('#dialog-program-status').textContent=message;document.dispatchEvent(new CustomEvent('zaya:program-change'));}
  ...
  $('#program-items').addEventListener('change',e=>{const id=e.target.dataset.dayId;if(!id)return;const day=e.target.value;store.setDay(id,day);changed(`${byId.get(id).title}: ${day||'gün henüz seçilmedi'}.`);$(`[data-day-id="${id}"]`).focus({preventScroll:true});});
  ...
  new IntersectionObserver(([entry])=>document.body.classList.toggle('at-planner',entry.isIntersecting),{threshold:0}).observe(form);
  render();
});
